...
export class CategoriaModuloInsertComponent {
  ...
  // ngOnInit(): void {
  //   this.categoriaModuloService.getCategoriaModulo().subscribe((data: any) => {
  ...
  insertar(): void {
    ...
    this.categoriaModuloService
      .insertCategoriaModulo(nuevoCatModulo)
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          console.error(
            "Hubo un error al insertar la categoria del modulo",
            error
          );
        },
      });
